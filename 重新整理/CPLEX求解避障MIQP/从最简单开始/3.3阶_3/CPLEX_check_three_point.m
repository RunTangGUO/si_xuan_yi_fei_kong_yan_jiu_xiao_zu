...
for i=1:2
    Constraints=[Constraints,AIneq(i,:)*x1<=bineq(i)];%+Aineq2(i,:)*x2
end
...
for i=1:1:2
     for j=1:1:11
         rTp(i,j)=subs(rTp(i,j),rTp(i,j),X1((i-1)*11+j,1));
     end
 end
 rTp=double(rTp);
 rT1=sym([0;0]);
for i=1:1:11
    rT1(1,1)=subs(rT1(1,1)+rTp(1,i)*p(i,1));
    rT1(2,1)=subs(rT1(2,1)+rTp(2,i)*p(i,1));
end
% hold on;
