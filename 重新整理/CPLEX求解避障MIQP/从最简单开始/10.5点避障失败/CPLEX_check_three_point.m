...
for i=1:8
    Constraints=[Constraints,AIneq(i,:)*x1<=bineq(i)];
end
% for i=27:30
...
for i=1:1:10
    ...
    rT4(1,1)=rT4(1,1)+rTp(7,i)*p(i,1);
    rT4(2,1)=rT4(2,1)+rTp(8,i)*p(i,1);
end
x0=zeros(100,1);
y0=zeros(100,1);
for i=1:100
...
y0=zeros(100,1);
for i=1:100
    x0(i,1)=double(subs(rT4(1,1),'x',i/100));
    y0(i,1)=double(subs(rT4(2,1),'x',i/100));
end
plot(x0,y0,'-.sr');
hold on;
x=1;
...
%»æÍ¼
line(x,y)
ax = double(subs(rT3(1,1),'x',1))-0.225;
ay = double(subs(rT3(2,1),'x',1))-0.225;
%ÉèÖÃ¾ØÐÎ³¤¿í
l = 0.45;
w = 0.45;
x = [ax,ax+l,ax+l,ax,ax];
...
%»æÍ¼
line(x,y)
ax = double(subs(rT4(1,1),'x',0.4))-0.225;
ay = double(subs(rT4(2,1),'x',0.4))-0.225;
%ÉèÖÃ¾ØÐÎ³¤¿í
l = 0.45;
w = 0.45;
x = [ax,ax+l,ax+l,ax,ax];
